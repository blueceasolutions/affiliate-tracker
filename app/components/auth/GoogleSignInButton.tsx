...
interface GoogleSignInButtonProps {
  /** Where Supabase should redirect after OAuth completes */
  redirectTo?: string
}

/**
 * GoogleSignInButton — triggers Supabase Google OAuth flow.
 * Single responsibility: Google SSO invocation only.
 */
export function GoogleSignInButton({ redirectTo }: GoogleSignInButtonProps) {
  ...
  return (
    <button
      ...
      {/* Google "G" logo */}
      {isLoading ? (
        <div className='animate-spin rounded-full h-4 w-4 border-b-2 border-slate-900 dark:border-slate-200'></div>
      ) : (
        <svg
          width='18'
          height='18'
